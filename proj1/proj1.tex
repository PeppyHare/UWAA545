...
\title{Preliminary Code – Kinetic Modeling:\\Vlasov-Poisson PIC}% Force line breaks with \\
...
\begin{abstract}
As a preparatory step towards a Vlasov-Poisson particle in cell model, the code presented here models streaming, non-interacting particles in one dimension. The model allows particles to be plotted in phase space over time, either as snapshots or as an animation. Spatial coordinates are subject to periodic boundary conditions. The total kinetic energy is computed at each time step to verify that total energy is conserved over time.
% \begin{description}
% \item[Usage]
...
One of the earliest practical computational models for laboratory plasmas, the particle in cell (PIC) method tracks a number of sampled ``particles'' in their own Lagrangian reference frames, computing the long-range electromagnetic forces at a limited set of Eulerian grid points (``cells''). By computing long-range forces on a grid and calculating local particle-particle collisions separately, the PIC method is able to capture the overall behavior of the plasma without the need for computing the full $N^2$ particle-particle interactions.

The governing equations for our PIC model for each particle $i$ are:

\begin{eqnarray}
\dv{\vec v_i}{t} & =  & \left( \frac{q_i}{m_i} \right) (\vec E + \vec v_i \cross \vec B)\nonumber \\
 & & + \sum_{j \neq i} \left. \dv{\vec v_{ij}}{t} \right|_{coll} \delta (\vec r_i - \vec r_j) \\
\dv{\vec r_i}{t} & = & \vec v_i
\end{eqnarray}

where $\vec E$ and $\vec B$ are the electromagnetic fields computed at the closest spatial grid location to particle $i$, and $\left. \dv{\vec v_{ij}}{t} \right|_{coll}$ is a collisional term which captures the effect of particle-particle collisions. In this very simplified model, we will disregard both the long range electromagnetic forces and the short range collisions, leaving us simply with the governing equations:

\begin{eqnarray}
\dv{\vec v_i}{t} = 0 \label{eq:dv}\\
\dv{\vec r_i}{t} = \vec v_i\label{eq:dr}
\end{eqnarray}


\section{Implementation}
The model is implemented as a Python script, making use of the \texttt{numpy} package for vector operations and the \texttt{matplotlib} package for visualizations. The \texttt{numba} package is used to transform computationally intensive routines into fast machine code. The implementation code is provided as the script \texttt{pic1.py} along with instructions for installing and running the code and descriptions of the various configurable parameters.

\subsection{Initial Conditions}

The initial particle distribution is randomly distributed within the ranges defined by the parameters \texttt{[x\_min, x\_max]} and \texttt{[v\_min,v\_max]}. The initial particle position distribution is randomly uniform, and the initial particle velocity distribution is Maxwellian. The full width at half maximum of the velocity distribution is determined by the parameter \texttt{v\_fwhm}. The total number of particles modeled is determined by the parameter \texttt{n}.

Example plots of the initial state are shown in Figure \ref{fig:initial_hist} for various values of \texttt{n}. The random distributions are generated using the standard Python \texttt{random} library. As such, the initial seed can be set using \texttt{random.seed()} to consistently reproduce the same initial distribution.


\begin{figure}[h]
\includegraphics[width=0.9\linewidth]{proj1/initial_hist_128_particles.pdf}
\includegraphics[width=0.9\linewidth]{proj1/initial_hist_512_particles.pdf}
\includegraphics[width=0.9\linewidth]{proj1/initial_hist_2048_particles.pdf}
\caption{\label{fig:initial_hist}Initial state for $N=[128, 512, 2048]$ particles. Particle position is uniformly distributed in $[-2\pi, 2\pi]$, and particle velocity is initially a Maxwellian with a full width at half maximum of $2$ and bounds at $[-5, 5]$.}
\end{figure}

\subsection{Time Step}

The governing equations \ref{eq:dv} and \ref{eq:dr} are a very simple linear system of equations. As such, a very simple first-order Euler time step is sufficient to produce $x_i (t+\Delta t), v_i (t + \Delta t)]$ from $[x_i (t), v_i (t)]$:
...
At each step, we apply periodic boundary conditions on $x_i (t)$ such that if $(x_i (t) + v_i (t) \Delta t > x_{max})$, then $x_i (t + \Delta t) = x_i (t) + v_i (t) \Delta t - (x_{max} - x_{min})$. A similar periodic boundary condition is applied at $x_{min}$. The value of $\Delta t$ is determined by the parameter \texttt{dt}. We also store the previous value of $[x_i (t), v_i (t)]$ at each time step for diagnostic purposes. The total kinetic energy of the system is computed at each time step as:

\begin{equation}
KE = \sum_{i} \frac{1}{2} m_i v_i ^2
\end{equation}

where $m_i$ is set to 1 for all particles. This allows us to visualize deviations in the total kinetic energy over time. The full time evolution procedure is implemented in the \texttt{time\_step()} function and just-in-time compiled to machine code using \texttt{numba} to optimize the performance.

\subsection{Diagnostics}

The model stores the full state of all particles at each time step within an array. This allows for complete diagnostics of the state of all particles at any time. Particle traces may be generated from the stored history using the optional \texttt{trace\_particles} flag, as shown in Figure . An optional \texttt{animate\_phase\_space} flag will generate a live animation of all particles in phase space over time, or snapshots of the phase space can be plotted as shown in Figure \ref{fig:snapshots_512}

\begin{figure}
\includegraphics[width=0.9\linewidth]{proj1/traces_128_particles.pdf}
\caption{\label{fig:traces_128}Traces of particle positions in phase space over time. Model parameters are $N=128$, $\Delta t = 0.01$, $t_{max} = 2 \pi$.}
\end{figure}

\begin{figure}
\includegraphics[width=0.9\linewidth]{proj1/snapshots_512_particles.pdf}
\caption{\label{fig:snapshots_512}Snapshots of particles in phase space at three different points in time, generated from the state history. Model parameters are $N=512$, $\Delta t = 0.01$.}
\end{figure}

\begin{figure*}
\includegraphics[width=0.7\linewidth]{proj1/d_ke_comparison.pdf}
\caption{\label{fig:ke_comparison}Kinetic energy conservation diagnostic, performed for various values of $N$ and $\Delta t$. These trials were evolved from $t=0$ to $t=8 \pi$.}
\end{figure*}

The total kinetic energy of all particles calculated at each time step is also stored within an array for subsequent diagnostics, as described in the previous section. Deviations from the initial kinetic energy of the initialized system are indicative of non-conservative phenomena. In the absence of collisions, the kinetic energy should be constant. An optional \texttt{compare\_ke} flag performs a more thorough analysis of the energy conservation properties by running the simulation using various values of $\Delta t$ and plotting the resulting kinetic energy diagnostics. As expected for our simple collision-less model, the kinetic energy remains constant over time for various values of $N$ and $\Delta t$, as shown in Figure \ref{fig:ke_comparison}.

\subsection{Next Steps}

With the state initialization and diagnostic groundwork in place, the next order of business will be the re-introduction of the long-range electromagnetic forces. The particles themselves are the sources for $\vec E$ and $\vec B$. A weighting procedure will be used to map particles onto a spatial grid, at which point the fields can be solved using a finite difference or finite element scheme.
